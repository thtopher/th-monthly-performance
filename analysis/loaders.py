...
class HarvestHoursLoader:
    ...
    def load(self) -> pd.DataFrame:
        ...
        # Try to find project/client name (optional)
...
class PnLLoader:
    ...
    def load(self) -> pd.DataFrame:
        ...
        for idx, row in df.iterrows():
            account_name = str(row.iloc[0]).strip()  # Account name in first column
            amount = row.iloc[total_col_idx]

            # Skip non-numeric amounts (headers, totals, etc.)
            if pd.isna(amount):
                continue
            try:
            ...
            # CRITICAL: Exclude income/revenue lines and subtotals
        ...
        # Log excluded lines
        if excluded:
            print(f"[INFO] ✓ Excluded {len(excluded)} income/subtotal lines from overhead pools:")
            for acc in excluded[:5]:
                print(f"  - {acc}")
            if len(excluded) > 5:
    ...
    def _should_exclude_pnl_line(self, account_name: str) -> bool:
        """
        Exclude income/revenue lines, subtotals, and aggregations from overhead pools.

        Only atomic expense accounts should be included in allocatable pools.
        This prevents inflating pools with income, subtotals, or double-counted aggregations.
        """
        account_lower = account_name.lower()

        # Exclude all "Total - " lines (subtotals and section totals)
        if account_name.startswith('Total - ') or account_name.startswith('Total -'):
            return True

        # Exclude known income/revenue accounts
        income_keywords = [
            'sales',
            'fixed fee',
            'recurring revenue',
        ...
        # Exclude standalone "Other" (too generic, likely income or subtotal)
        if account_name.strip().lower() == 'other':
            return True

        # Exclude known aggregation/summary lines
        summary_lines = [
            'gross profit',
            'net income',
            'net ordinary income',
